...
function ExtraItem({ e }: { e: { text: string; note: string } }) {
  return (
    <div style={{ padding: "14px 0", borderBottom: `0.5px solid rgba(28,44,53,0.1)` }}>
      {e.text && <div style={{ fontSize: 14, color: TEXT, marginBottom: 6 }}>{e.text}</div>}
...
export default function Okinawa() {
  ...
  async function handleShare() {
    const blob = await buildPhotoGridShareCard(GALLERY.map(g => g.url), { kicker: "TRAVEL ARCHIVE", title: "Okinawa", backdropColor: BG, footerTop: "Spring 2025", footerBottom: "5 Days" });
    await shareOrCopyLink({
      title: "Okinawa — Matt Travels",
      text: "Spring 2025, Apr 11–15",
      url: window.location.href,
      fileBlob: blob,
      filename: "okinawa-story.png",
      fallbackImgUrl: "https://res.cloudinary.com/dydhvvubl/image/upload/f_auto,q_auto/v1778843907/OKI1_kvpgjq.jpg",
