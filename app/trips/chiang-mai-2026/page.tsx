...
export default function ChiangMai() {
  ...
  async function handleShare() {
    const blob = await buildPhotoGridShareCard(GALLERY.map(g => g.url), { kicker: "TRAVEL ARCHIVE", title: "Chiang Mai", backdropColor: BG, footerTop: "Spring 2026", footerBottom: "7 Days" });
    await shareOrCopyLink({
      title: "Chiang Mai — Matt Travels",
      text: "Feb 2026, Chiang Mai",
      url: window.location.href,
      fileBlob: blob,
      filename: "chiangmai-story.png",
      fallbackImgUrl: "https://res.cloudinary.com/dydhvvubl/image/upload/f_auto,q_auto/v1778430032/CM1_syxfa2.jpg",
          ...
          <div style={{ display: "flex", gap: isMobile ? 6 : 10 }}>
            {[col1, col2, col3].map((col, ci) => (
              <div key={ci} style={{ flex: 1 }}>
                {col.map((img, i) => (
                  <GalleryImage key={i} img={img} index={ci * 3 + i} onClick={() => setLightbox(img.url)} isMobile={isMobile} isTouch={isTouch} />
                ))}
              </div>
            ))}
          </div>
        ...
        >
          ...
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "repeat(2, 1fr)" : "repeat(4, 1fr)", gap: isMobile ? 24 : 32 }}>
            {[
              ...
              { icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5}><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>, label: "WITH", value: "Family", sub: "Chinese New Year trip" },
            ].map((m, i) => (
              <div key={i}>
