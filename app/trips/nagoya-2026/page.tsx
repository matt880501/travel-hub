...
function GalleryImage({ img, index, onClick, isMobile, isTouch }: { img: typeof GALLERY[0]; index: number; onClick: () => void; isMobile: boolean; isTouch: boolean }) {
  ...
  return (
    ...
    >
      <img src={img.url} style={{ width: "100%", height: "auto", display: "block", transition: "transform 0.6s cubic-bezier(0.25,0.1,0.25,1)", transform: (!isTouch && active) ? "scale(1.03)" : "scale(1)" }} />
      <AnimatePresence>
        {show && (
          <motion.div
            initial={{ opacity: 0 }}
...
export default function Nagoya() {
  ...
  async function handleShare() {
    const blob = await buildPhotoGridShareCard(GALLERY.map(g => g.url), { kicker: "TRAVEL ARCHIVE", title: "Nagoya", subtitle: "Takayama · Inuyama" });
    await shareOrCopyLink({
      title: "Nagoya · Takayama · Inuyama — Matt Travels",
      text: "Winter 2026, 6 days in Japan",
      url: window.location.href,
      fileBlob: blob,
      filename: "nagoya-story.png",
      fallbackImgUrl: "https://res.cloudinary.com/dydhvvubl/image/upload/f_auto,q_auto/v1778669488/NG1_cfqqat.jpg",
    ...
    <div style={{ minHeight: "100vh", background: BG, fontFamily: "-apple-system, 'Helvetica Neue', sans-serif", color: TEXT }}>
      ...
      <div ref={heroRef} style={{ position: "relative", overflow: "hidden", backgroundColor: "#0d1a28" }}>
        ...
        <button
          onClick={handleShare}
          aria-label="Share"
          style={{ position: "absolute", top: isMobile ? 68 : 90, right: isMobile ? 16 : 40, zIndex: 2, width: 34, height: 34, borderRadius: "50%", background: "rgba(255,255,255,0.12)", backdropFilter: "blur(10px)", border: "1px solid rgba(255,255,255,0.2)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}
        >
      ...
      <div style={{ maxWidth: 860, margin: "0 auto", padding: isMobile ? "60px 20px" : "80px 40px" }}>
          ...
          <div style={{ display: "flex", gap: isMobile ? 6 : 10 }}>
            {[col1, col2, col3].map((col, ci) => (
              <div key={ci} style={{ flex: 1 }}>
                {col.map((img, i) => (
                  <GalleryImage key={i} img={img} index={ci * 6 + i} onClick={() => setLightbox(img.url)} isMobile={isMobile} isTouch={isTouch} />
                ))}
              </div>
            ))}
          </div>
        ...
        >
          ...
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "repeat(2, 1fr)" : "repeat(4, 1fr)", gap: isMobile ? 24 : 32 }}>
            {[
              { icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5}><path d="M14 14.76V3.5a2.5 2.5 0 0 0-5 0v11.26a4.5 4.5 0 1 0 5 0z"/></svg>, label: "TEMPERATURE", value: "2°C — 15°C", sub: "Comfortably cold" },
              { icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5}><circle cx="12" cy="12" r="5"/><line x1="12" y1="1" x2="12" y2="3"/><line x1="12" y1="21" x2="12" y2="23"/><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"/><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"/><line x1="1" y1="12" x2="3" y2="12"/><line x1="21" y1="12" x2="23" y2="12"/><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"/><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"/></svg>, label: "MOOD", value: "Slow / Quiet Towns", sub: "Snow country pace" },
              { icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5}><circle cx="12" cy="12" r="10"/><path d="M2 12h20M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>, label: "SEASON", value: "Late Winter", sub: "Shirakawago in white" },
              { icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5}><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>, label: "WITH", value: "Charlotte", sub: "Just the two of us" },
            ].map((m, i) => (
              <div key={i}>
